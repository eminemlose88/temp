...
async function refreshSummary(){
  ...
  if(data.ok){sumEl.textContent=`总账：${data.data.total||0} KRW · 用户数：${data.data.users||0}`}
}
...
qBtn.onclick=async()=>{
  ...
  const head=document.createElement('div');head.className='subtitle';head.textContent=`合计：${total} KRW`
